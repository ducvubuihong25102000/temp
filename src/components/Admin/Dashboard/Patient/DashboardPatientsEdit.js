...
export default function DashboardPatientsEdit(props) {
  const { enqueueSnackbar } = useSnackbar();
  const createForm = useRef();
  const dispatch = useDispatch();
  const update = useSelector((state) => state.patients.updatePatients);
  const job_content_vi = useSelector(
    (state) => state.patients.updatePatients.job_content_vi
  );
  const job_content_en = useSelector(
    (state) => state.patients.updatePatients.job_content_en
  );
